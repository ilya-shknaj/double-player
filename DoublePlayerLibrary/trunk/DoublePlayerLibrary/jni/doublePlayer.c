...
//				GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE, desired_position);
	...
	seek_event = gst_event_new_seek(data->rate, GST_FORMAT_TIME,
			...
			desired_position, GST_SEEK_TYPE_NONE, 0);
...
static jboolean gst_native_class_init(JNIEnv* env, jclass klass) {
	...
	on_video_finished = (*env)->GetMethodID(env, klass, "onVideoFinished",
			"()V");
	on_set_rate_finished = (*env)->GetMethodID(env, klass, "onSetRateFinished",
			"()V");

	if (!custom_data_field_id || !set_message_method_id
			|| !on_gstreamer_initialized_method_id
			|| !on_media_size_changed_method_id
			|| !set_current_position_method_id || !on_video_finished
...
void gst_seek_to_position(JNIEnv *env, jobject thiz, gint milliseconds) {
	...
	GstEvent *seek_event = gst_event_new_seek(data->rate, GST_FORMAT_TIME,
			GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE, GST_SEEK_TYPE_SET,
			desired_position, GST_SEEK_TYPE_NONE, 0);
...
void gst_native_set_fragment(JNIEnv *env, jobject thiz, gint startPosition,
		gint finishPosition) {
	...
	g_print("set fragment start position %d finish position %d", startPosition,
			finishPosition);
	...
	GstEvent *seek_event = gst_event_new_seek(data->rate, GST_FORMAT_TIME,
			...
			GST_SEEK_TYPE_SET,
			start_position, GST_SEEK_TYPE_SET, finish_position);
...
static JNINativeMethod native_methods[] = { { "nativeInit", "()V",
		...
		{ "nativeClassInit", "()Z", (void *) gst_native_class_init }, {
				...
				"nativeSetRepeatMode", "(Z)V",
				(void *) gst_native_set_repeat_mode }, { "nativeSeekToPosition",
				"(I)V", (void *) gst_seek_to_position }, { "nativeSetFragment",
				"(II)V", (void *) gst_native_set_fragment } };
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
	...
	klass = (*env)->FindClass(env,
			"by/gravity/doublexplayer/core/fragment/NativeVideoFragment");
