...
public class Zip4jUtil {
	...
	public static long javaToDosTime(long time) {
		...
		return (year - 1980) << 25 | (cal.get(Calendar.MONTH) + 1) << 21 | cal.get(Calendar.DATE) << 16 | cal.get(Calendar.HOUR_OF_DAY) << 11
...
//		if (isUTF8) {
			return new StringBuilder(new String(data, InternalZipConstants.CP866)).toString();
//		} else {
...
//		}
	...
	/**
	 * Returns a string in Cp850 encoding from the input bytes. If this encoding
	 * is not supported, then String with the default encoding is returned.
	 * 
	 * @param data
	 * @return String
	 */
	 ...
	 *            - list to be checked
	 * @param type
	 *            - type of elements to be present in the list (ex: File,
	 *            String, etc)
	...
	/**
	 * returns the length of the string by wrapping it in a byte buffer with the
	 * appropriate charset of the input string and returns the limit of the byte
	 * buffer
	 * 
	 * @param str
		...
		if (numberOfThisDisk == 0) {
			...
			for (int i = 0; i <= numberOfThisDisk; i++) {
				if (i == numberOfThisDisk) {
					...
					partFile = (zipFileName.indexOf(".") >= 0) ? currZipFile.substring(0, currZipFile.lastIndexOf(".")) : currZipFile + fileExt
							+ (i + 1);
